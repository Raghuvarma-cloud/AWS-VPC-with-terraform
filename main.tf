...
resource "aws_subnet" "public_subnet" {
  ...
  tags = {
    Name = "new Public subnet ${count.index + 1}"
  }
}
...
resource "aws_subnet" "private_subnet" {
  ...
  availability_zone = element(var.vpc_availability_zones, count.index)
  tags = {
    Name = "new Private subnet ${count.index + 1}"
  }
}
...
resource "aws_nat_gateway" "yt-nat-gateway" {
  ...
  depends_on    = [aws_internet_gateway.igw_vpc]
  tags = {
    Name = "new-Nat Gateway"
  }
}

//8. Route table for Private subnet
resource "aws_route_table" "new_route_table_private_subnet" {
  vpc_id = aws_vpc.custom_vpc.id
  route {
    cidr_block = "0.0.0.0/0"
...
resource "aws_route_table_association" "private_subnet_association" {
  route_table_id = aws_route_table.new_route_table_private_subnet.id
  count          = length(var.vpc_availability_zones)
